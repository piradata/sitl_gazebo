...
# -*- coding: utf-8 -*-
"""
Created on Sat Aug 22 11:14:00 2020
...
class YOLOff(object):
    ...
    def detecta_img(self, imagem):
        ...
        for i, c in reversed(list(enumerate(classes_saida))):
            ...
            lista_de_objetos.append([cima, esquerda, baixo, direita, meio_v, meio_h, rotulo, scores])
...
if __name__=="__main__":
    yolo = YOLOff()
    for i in range(581):
        imagem = str('/home/gabryelsr/Pictures/databank/ships/image'+str(i)+'.png')
        arq_img_detectadas='/home/gabryelsr/Pictures/databank/Object-Detection-Metrics-master/detections/'
        #imagem = str('/home/gabryelsr/Pictures/databank/ROS_imgs/ros'+str(i)+'.png')
        r_imagem, lista_de_objetos = yolo.detecta_imagem(imagem)
        #print(lista_de_objetos)
        arquivos=""
        for lista in lista_de_objetos:
            arquivos += (str(lista[6])+" "+str(lista[1])+" "+str(lista[0])+" "+str(lista[3])+" "+str(lista[2])+"\n")
            #print(arquivos)
            arq=arq_img_detectadas+"image"+str(i)+".txt"
            gt = open(arq, "w") 
            gt.write(arquivos) 
            gt.close()
        ...
        #cv2.imwrite('/home/gabryelsr/Pictures/databank/ROS_imgs/processado_yoloff_ros'+str(i)+'.png', cv2.cvtColor(r_imagem, cv2.COLOR_BGR2RGB))
