...
# float32 yaw_rate
    ...
    def missao_pt2(self, status_decolagem, h=15):
        ...
        if status_decolagem==True:
            ...
            log_y=[]
            log_vel=[]
            while self.arm_state and (rospy.is_shutdown() is False):
                #self.local_target_pub.publish(self.cur_target_pose)
                            ...
                            if obj_x>centrox:
                                ...
                                if teta>0:
                                    ...
                                    if tetad<-180:
                                        tetad=tetad+360
                                print('######################################### NOVA ORIENTACAO:', tetad)
                            while abs(tetad-self.current_heading*180/np.pi)>=2 or abs(h-self.local_pose.pose.position.z)>=0.1:
                                self.cur_target_pose=self.controle_orientacao(tetad, h)
                                #self.local_target_pub.publish(self.cur_target_pose)
                                time.sleep(0.2)
                                teta=self.current_heading*180/np.pi
                                print('Orientacao atual do drone:', teta)
                                print('Orientacao desejada:', tetad)
                                if indicador=='D':
