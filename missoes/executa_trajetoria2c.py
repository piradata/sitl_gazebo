	...
	#rospy.init_node("offboard_node")
        ...
        while self.arm_state and self.offboard_state and (rospy.is_shutdown() is False):
            ...
            if j < len(pontos):
                ...
                yt=pontos[j][1]
                zt=pontos[j][2]
                yawt=np.arctan((self.local_pose.pose.position.y-yt)/(self.local_pose.pose.position.x-xt))#np.float32(pontos[j][3]*np.pi/180)
                ...
                self.controle_gimbal=self.controle_gimbal_camera(0, -90, 0)
                self.controle_gimbal_pub.publish(self.controle_gimbal)
            if np.absolute(x-xt)<=0.05 and np.absolute(y-yt)<=0.05 and np.absolute(z-zt)<=0.05:
             			j=j+1
             			print(j)
            		#else:
            			#self.hover()
...
if __name__ == '__main__':
    ...
    v=1
    ang=0
    pontos=[[0,0,h],[0,-1,h],[18,-1,h],[18,-3,h],[-5,-3,h],[-5,-6,h],[18,-6,h],
            [18,-9,h],[-5,-9,h],[-5,-12,h],[18,-12,h]]
    #con.navegue2(v, ang)
    con.navegue(pontos)
